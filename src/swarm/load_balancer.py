...
class LoadBalancer:
    ...
    def select_sub_orchestrator(self, exclude: Optional[List[UUID]] = None) -> Optional[UUID]:
        ...
        available_orchestrators = [
            sub_id for sub_id in self.load_metrics.keys() if sub_id not in exclude
        ]
    ...
    def _select_least_loaded(self, available: List[UUID]) -> UUID:
        ...
        return min(available, key=lambda sub_id: self.load_metrics[sub_id].current_tasks)
    ...
    def record_task_completion(
        self, sub_orchestrator_id: UUID, duration: float, success: bool
    ) -> None:
        ...
        if metrics.avg_task_duration == 0:
            ...
            metrics.avg_task_duration = metrics.avg_task_duration * 0.9 + duration * 0.1

        # Update success rate
        if metrics.success_rate == 0:
            metrics.success_rate = 1.0 if success else 0.0
        else:
            metrics.success_rate = metrics.success_rate * 0.95 + (1.0 if success else 0.0) * 0.05
    ...
    def get_overall_load(self) -> Dict[str, Any]:
        ...
        avg_memory = sum(m.memory_usage for m in self.load_metrics.values()) / len(
            self.load_metrics
        )
        avg_success = sum(m.success_rate for m in self.load_metrics.values()) / len(
            self.load_metrics
        )
