...
class TaskDistributor:
    ...
    def register_sub_orchestrator(
        self, sub_orchestrator_id: UUID, capabilities: List[str], max_capacity: int = 10
    ) -> None:
        ...
        logger.info(
            f"Registered sub-orchestrator {sub_orchestrator_id} with capabilities: {capabilities}"
        )
    ...
    def distribute_task(self, task: Task) -> Optional[UUID]:
        ...
        matching_orchestrators = [
            sub_id
            for sub_id, caps in self.sub_orchestrator_capabilities.items()
            if all(cap in caps.capabilities for cap in task.required_capabilities)
    ...
    def get_sub_orchestrator_status(
        self, sub_orchestrator_id: UUID
    ) -> Optional[SubOrchestratorCapabilities]:
    ...
    def get_distribution_stats(self) -> Dict[str, Any]:
        ...
        total_load = sum(caps.current_load for caps in self.sub_orchestrator_capabilities.values())
