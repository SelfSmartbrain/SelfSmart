...
from src.architecture.architecture_benchmark import (
...
async def calculate_composite_fitness(
    ...
    if baseline.throughput_rps > 0:
        throughput_improvement = (
            candidate.throughput_rps - baseline.throughput_rps
    ...
    if baseline.memory_usage_mb > 0:
        memory_improvement = (
            baseline.memory_usage_mb - candidate.memory_usage_mb
        ) / baseline.memory_usage_mb
        score += memory_improvement * 15

    if baseline.cpu_usage_percent > 0:
        cpu_improvement = (
            baseline.cpu_usage_percent - candidate.cpu_usage_percent
