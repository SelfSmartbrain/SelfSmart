...
class ArchitectureRegistry:
    ...
    async def register_version(
        self, version_name: str, metadata: Dict[str, Any] = None
    ) -> uuid.UUID:
        ...
        result = await self.session.execute(query, {"version": version_name, "metadata": metadata})
    ...
    async def save_snapshot(self, version_id: uuid.UUID, snapshot_hash: str) -> uuid.UUID:
        ...
        result = await self.session.execute(
            query, {"version_id": version_id, "snapshot_hash": snapshot_hash}
        )
    ...
    async def register_components(
        self, snapshot_id: uuid.UUID, components: list[Dict[str, Any]]
    ) -> None:
        if not components:
            return
