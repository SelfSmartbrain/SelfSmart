...
class PerformanceProfiler:
    ...
    async def start_profiling(self, entity_id: UUID, entity_type: str) -> None:
        profile = ExecutionProfile(
            entity_id=entity_id, entity_type=entity_type, start_time=datetime.utcnow()
        )
    ...
    async def stop_profiling(self, entity_id: UUID) -> Optional[ExecutionProfile]:
        ...
        logger.info(
            f"Stopped profiling for {profile.entity_type} {entity_id}. Time: {profile.execution_time_ms}ms"
        )
        return profile
