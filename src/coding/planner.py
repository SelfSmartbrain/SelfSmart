...
class Planner:
    ...
    def create_plan(self, goal: str, context: Optional[Dict] = None) -> ExecutionPlan:
        ...
        plan = ExecutionPlan(task_type=task_type, goal=goal, context=context or {})
    ...
    def _plan_generic(self, plan: ExecutionPlan):
        ...
        plan.add_step(
            ExecutionStep(
                step_type=StepType.READ, description="Read relevant files", dependencies=["locate"]
            )
        )

        plan.add_step(
            ExecutionStep(
                step_type=StepType.GENERATE, description="Generate solution", dependencies=["read"]
            )
        )

        plan.add_step(
            ExecutionStep(
                step_type=StepType.APPLY, description="Apply solution", dependencies=["generate"]
            )
        )

        plan.add_step(
            ExecutionStep(
                step_type=StepType.TEST, description="Test the solution", dependencies=["apply"]
            )
        )
