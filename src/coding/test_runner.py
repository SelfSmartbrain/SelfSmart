...
class TestRunner:
    ...
    def run_tests(
        self, framework: Optional[TestFramework] = None, args: Optional[List[str]] = None
    ) -> TestResult:
        ...
        if framework is None:
            return TestResult(
                framework=TestFramework.PYTEST, success=False, errors=["No test framework detected"]
            )
    ...
    def _detect_framework(self) -> Optional[TestFramework]:
        ...
        if (
            (self.repository_path / "pytest.ini").exists()
            or (self.repository_path / "pyproject.toml").exists()
            or (self.repository_path / "setup.py").exists()
        ):
    ...
    def _execute_command(self, command: List[str], framework: TestFramework) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                framework=framework, success=False, errors=["Test execution timed out"]
            )
        except Exception as e:
            return TestResult(framework=framework, success=False, errors=[str(e)])
    ...
    def _parse_generic_output(
        self, output: str, success: bool, framework: TestFramework
    ) -> TestResult:
    ...
    def run_specific_test(
        self, test_path: str, framework: Optional[TestFramework] = None
    ) -> TestResult:
