...
class LongHorizonMetrics:
    ...
    def update(self, result: BenchmarkResult):
        ...
        self.success_rate = (
            self.successful_tasks / self.completed_tasks if self.completed_tasks > 0 else 0
        )
...
class LongHorizonValidator:
    ...
    def setup_benchmark(self):
        ...
        if total_available < self.target_tasks:
            print(
                f"Warning: Only {total_available} tasks available, but target is {self.target_tasks}"
            )
    ...
    def run_validation(
        ...
        tasks_to_run = all_tasks[
            self.current_task_index : self.current_task_index + self.target_tasks
        ]
    ...
    def get_progress_summary(self) -> Dict[str, Any]:
        ...
        return {
            "progress": (
                (self.metrics.completed_tasks / self.target_tasks * 100)
...
class ValidationOrchestrator:
    ...
    def create_validation_run(
        self, run_id: str, target_tasks: int = 100, checkpoint_interval: int = 10
    ...
    def run_comparison(self, configurations: List[Dict[str, Any]]) -> Dict[str, LongHorizonMetrics]:
...
def run_standard_validation(benchmark_root: str, target_tasks: int = 100):
    ...
    validator = LongHorizonValidator(
        benchmark_root=benchmark_root, target_tasks=target_tasks, checkpoint_interval=10
    )
