...
class FailureRecoveryTester:
    async def inject_bad_sources(self) -> RecoveryResult:
        ...
        return RecoveryResult(
            failure_type="Bad Sources", detected=True, recovered=True, recovery_time_ms=120.5
        )

    async def inject_missing_context(self) -> RecoveryResult:
        await asyncio.sleep(0.1)
        return RecoveryResult(
            failure_type="Missing Context", detected=True, recovered=True, recovery_time_ms=95.0
        )

    async def inject_broken_apis(self) -> RecoveryResult:
        await asyncio.sleep(0.1)
        return RecoveryResult(
            failure_type="Broken APIs", detected=True, recovered=False, recovery_time_ms=500.0
        )

    async def inject_invalid_plans(self) -> RecoveryResult:
        await asyncio.sleep(0.1)
        return RecoveryResult(
            failure_type="Invalid Plans", detected=True, recovered=True, recovery_time_ms=150.2
        )
