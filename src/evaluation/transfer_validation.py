...
class TransferValidator:
    def __init__(self):
        ...
        self.transferable_concepts = {
            "algorithms": {
                "weight": 0.3,
                "keywords": {"sort", "search", "graph", "tree", "sorting", "searching"},
            },
            "mathematics": {
                "weight": 0.25,
                "keywords": {
        ...
        self.domain_mappings = {
            "math_to_cs": {
                "source_domains": {"mathematics"},
                "target_domains": {"algorithms", "data_structures"},
                "transfer_factor": 0.7,
            },
            "cs_to_math": {
                "source_domains": {"algorithms", "data_structures"},
                "target_domains": {"mathematics"},
                "transfer_factor": 0.6,
            },
            "ml_to_software": {
                "source_domains": {"machine_learning"},
                "target_domains": {"algorithms", "optimization"},
                "transfer_factor": 0.5,
            },
        }
    ...
    def validate(self, predictions: List[Dict[str, str]]) -> Dict[str, float]:
        ...
        return {"transfer_score": round(final_score, 4), "details": details}

    def _calculate_transfer_bonus(
        self, source_domain: str, target_domain: str, outcome: str
    ) -> float:
        ...
        for mapping_key, mapping in self.domain_mappings.items():
            ...
            if source_match and target_match:
                ...
                if any(
                    word in outcome_words
                    for word in [
                        word
                        for domain in list(mapping["source_domains"])
                        + list(mapping["target_domains"])
    ...
    def _generate_validation_details(
        self, predictions: List[Dict[str, str]], overall_score: float, category_scores: dict
    ) -> dict:
        ...
        details = {
            ...
            "weakest_category": (
                min(category_scores, key=category_scores.get)
                if any(category_scores.values())
                else "none"
            ),
