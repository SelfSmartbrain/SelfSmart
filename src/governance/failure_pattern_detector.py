...
class FailurePatternDetector:
    ...
    def detect_patterns(self) -> List[FailurePattern]:
        ...
        successful = [
            d for d in self.decision_history if d.get("outcome", {}).get("success", False)
        ]
    ...
    def _detect_constraint_patterns(
        ...
        if len(fail_constraints) >= 3:
            ...
            avg_success_constraints = (
                sum(success_constraints) / len(success_constraints) if success_constraints else 0
            )
            ...
            if avg_fail_constraints > avg_success_constraints + 1:
                pattern = FailurePattern(
                    ...
                    failure_rate=len(fail_constraints)
                    / (len(fail_constraints) + len(success_constraints)),
