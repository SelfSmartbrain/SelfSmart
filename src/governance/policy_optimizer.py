...
class PolicyOptimizer:
    ...
    def analyze_policy_effectiveness(
        ...
        performance = {
            ...
            "most_common_violation": (
                violation_counts.most_common(1)[0] if violation_counts else None
            ),
        }
    ...
    def propose_optimization(
        ...
        if most_common_violation:
            ...
            if "risk" in violation_type.lower():
                optimization = self._propose_risk_parameter_tuning(
                    policy_id, current_policy, performance
                )
            elif "resource" in violation_type.lower():
                optimization = self._propose_resource_adjustment(
                    policy_id, current_policy, performance
                )
            else:
                optimization = self._propose_scope_adjustment(
                    policy_id, current_policy, performance
                )
    ...
    def _check_policy_conflict(
        ...
        risk_limits1 = [
            r.get("parameters", {}).get("max_risk") for r in rules1 if r.get("type") == "risk_limit"
        ]
        risk_limits2 = [
            r.get("parameters", {}).get("max_risk") for r in rules2 if r.get("type") == "risk_limit"
        ]
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        by_type = {
            opt_type.value: len(
                [o for o in self.optimizations.values() if o.optimization_type == opt_type]
            )
