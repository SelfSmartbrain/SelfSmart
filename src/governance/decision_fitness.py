...
class DecisionFitnessCalculator:
    ...
    def get_fitness_statistics(self) -> Dict[str, Any]:
        ...
        avg_fitness = (
            sum(f.overall_fitness for f in self.fitness_history.values()) / total_decisions
        )
        ...
        return {
            ...
            "high_fitness_decisions": len(
                [f for f in self.fitness_history.values() if f.overall_fitness > 0.8]
            ),
            "low_fitness_decisions": len(
                [f for f in self.fitness_history.values() if f.overall_fitness < 0.4]
            ),
        }
