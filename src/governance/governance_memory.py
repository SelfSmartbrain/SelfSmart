...
class GovernanceMemory:
    ...
    def get_records_in_range(
        ...
        return [r for r in self.records.values() if start_date <= r.timestamp <= end_date]
    ...
    def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        by_type = {
            r_type: len(self.get_records_by_type(r_type)) for r_type in self.records_by_type.keys()
        }
        ...
        return {
            ...
            "avg_records_per_decision": (
                sum(by_decision.values()) / len(by_decision) if by_decision else 0
            ),
        }
