...
class CausalAttributor:
    ...
    def _build_causal_chain(
        ...
        context = decision_data.get("context", {})
        chain.append(
            f"Decision made with context: {context.get('time_horizon', 'unknown')} time horizon"
        )
    ...
    def _identify_primary_cause(self, factors: List[CausalFactor]) -> Optional[str]:
        ...
        strongest = max(factors, key=lambda f: (strength_order.get(f.strength, 0), f.confidence))
    ...
    def _calculate_confidence(self, attribution: CausalAttribution) -> float:
        ...
        avg_factor_confidence = sum(f.confidence for f in attribution.causal_factors) / len(
            attribution.causal_factors
        )
        ...
        for attribution in self.attributions.values():
            for factor in attribution.causal_factors:
                ...
                factor_counts[key]["avg_strength"] += (
                    1
                    if factor.strength == CausalStrength.STRONG
                    else 0.5 if factor.strength == CausalStrength.MODERATE else 0.25
                )
    ...
    def get_attribution_statistics(self) -> Dict[str, Any]:
        ...
        avg_confidence = sum(a.confidence for a in self.attributions.values()) / total_attributions
