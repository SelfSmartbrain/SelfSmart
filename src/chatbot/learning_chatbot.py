...
class LearningChatbot:
    ...
    async def _generate_response(self, message: str) -> ChatResponse:
        ...
        try:
            ...
            # Extract sources
            sources = [
                result.get("metadata", {}).get("source_url", "") for result in search_results[:5]
            ]
    ...
    async def _generate_response_from_results(
        self, message: str, search_results: List[Dict[str, Any]], context: str
    ) -> str:
    ...
    def _template_based_response(
        self, message: str, relevant_content: List[Dict[str, Any]], context: str
    ) -> str:
        """Generate response using template-based approach"""
    ...
    def _generate_fallback_response(self, message: str) -> str:
        ...
        return (
            "I'm continuously learning from the internet to expand my knowledge base. "
    ...
    def _calculate_confidence(
        self, search_results: List[Dict[str, Any]], response_text: str
    ) -> float:
        ...
        avg_quality = sum(
            [result.get("metadata", {}).get("quality_score", 0) for result in search_results]
        ) / len(search_results)
    ...
    async def get_chatbot_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"is_learning": self.is_learning, "error": str(e)}
    ...
    async def get_knowledge_about_topic(self, topic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = {
                ...
                "topic_breakdown": {
                    topic_name: {
                        "count": len(results),
                        "avg_quality": sum(
                            [r.get("metadata", {}).get("quality_score", 0) for r in results]
                        )
                        / len(results),
            ...
            return {"topic": topic, "error": str(e), "total_results": 0}
        ...
        try:
            if 0 <= message_index < len(self.conversation_history):
                ...
                self.chat_stats["user_satisfaction"] = (
                    self.chat_stats["user_satisfaction"] * (self.chat_stats["total_messages"] - 1)
                    + rating
