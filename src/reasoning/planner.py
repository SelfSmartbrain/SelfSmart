...
try:
    ...
    from src.reasoning.reactive_replanner import (
        ReactiveRePlanner,
        ReplanContext,
        ReplanStrategy,
...
class Planner:
    ...
    async def _hierarchical_planning(
        ...
        """
        if not self._llm:
            raise ValueError("LLM client required for hierarchical planning")

        prompt = f"Decompose this goal into a strict JSON list of 3-5 sequential actions:\nGoal: {goal}\nContext: {context}"

        try:
            response = await self._llm.ainvoke(prompt)
            # Parse JSON and construct dynamic actions
            import json
            ...
            raw_actions = json.loads(raw_text)
            ...
            for i, act in enumerate(raw_actions):
                actions.append(
                    Action(
                        action_id=f"step_{i}",
                        description=act.get(
                            "description", act.get("title", f"Autogenerated step {i}")
                        ),
                        priority=10 - i,
            ...
            # Fallback to single action if LLM fails
            return [Action(action_id="fallback", description=f"Execute: {goal}")]
        ...
        try:
            ...
            while len(executed_actions) < len(plan.actions):
                ...
                if self._drift_monitoring_enabled:
                    drift_result = await self._check_and_handle_drift(
                        plan_id, len(executed_actions)
                    )
    ...
    async def _check_and_handle_drift(
        ...
        context["completed_actions"] = [
            a.action_id
            for a in self._plans[plan_id].actions
            if a.action_id in current_state.get("executed", set())
        ]
        context["remaining_actions"] = [
            a
            for a in self._plans[plan_id].actions
            if a.action_id not in current_state.get("executed", set())
        ]

        result = await self._replanning_coordinator.check_and_replan(
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "success_rate": (
                self._plans_completed / self._plans_created if self._plans_created > 0 else 0.0
            ),
