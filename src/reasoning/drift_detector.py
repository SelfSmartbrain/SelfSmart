...
class EnvironmentalDriftDetector:
    ...
    async def register_plan(
        ...
        logger.info(
            f"Registered plan {plan_id} for drift monitoring ({len(expected_states)} steps)"
        )
    ...
    async def check_drift(
        ...
        constraint_signals = await self._check_constraint_violations(
            plan_id, current_state, context
        )
        ...
        if significant:
            ...
            for s in significant:
                logger.warning(
                    f"  - {s.drift_type.value}: {s.description} (severity: {s.severity:.2f})"
                )
