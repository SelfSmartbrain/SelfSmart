...
class ScreenshotPipeline:
    ...
    async def capture_screenshot(
        ...
        try:
            ...
            metadata = ScreenshotMetadata(
                url=url, viewport_size=viewport, session_id=session_id, task_id=task_id
            )
    ...
    async def capture_screenshot_batch(
        ...
        tasks = [self.capture_screenshot(url, viewport, session_id=session_id) for url in urls]
        return await asyncio.gather(*tasks)

    async def process_screenshot(
        self, metadata: ScreenshotMetadata, extract_text: bool = True, detect_elements: bool = True
        ...
        analysis = await self.vision_processor.analyze_screenshot(
            image_data, extract_text=extract_text, detect_elements=detect_elements
        )
    ...
    async def get_interactive_elements(
        self, url: str, viewport: tuple[int, int] = (1920, 1080)
    ) -> List[Dict[str, Any]]:
    ...
    async def compare_screenshots(
        self, screenshot1: ScreenshotMetadata, screenshot2: ScreenshotMetadata
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "interactive_diff": len(analysis1[1].interactive_elements)
            - len(analysis2[1].interactive_elements),
