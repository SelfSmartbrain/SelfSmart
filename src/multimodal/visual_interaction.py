...
class VisualInteractionAgent:
    ...
    async def click_element(self, element: VisualElement, timeout: int = 5000) -> InteractionResult:
        ...
        try:
            ...
            return InteractionResult(
                success=True,
                action=InteractionAction(
                    action_type="click", element=element, coordinates=(center_x, center_y)
                ),
                metadata={
            ...
            return InteractionResult(
                success=False,
                action=InteractionAction(action_type="click", element=element),
                error_message=str(e),
            )

    async def type_text(
        self, element: VisualElement, text: str, clear_first: bool = True
    ) -> InteractionResult:
        ...
        try:
            ...
            return InteractionResult(
                success=True,
                action=InteractionAction(action_type="type", element=element, text=text),
            )
        except Exception as e:
    ...
    async def scroll_page(self, direction: str = "down", amount: int = 500) -> InteractionResult:
        """Scroll the page."""
        if not self._current_page:
            raise RuntimeError("No active page. Call navigate_to first.")
        ...
        try:
            ...
            return InteractionResult(
                success=True,
                action=InteractionAction(action_type="scroll", text=f"{direction} {amount}px"),
            )
        except Exception as e:
            logger.error(f"Error scrolling: {e}")
            return InteractionResult(
                success=False, action=InteractionAction(action_type="scroll"), error_message=str(e)
            )

    async def find_and_click(
        self, element_type: str, text_match: Optional[str] = None
    ) -> InteractionResult:
        ...
        # Filter by type
        matching_elements = [elem for elem in elements if elem.element_type == element_type]
