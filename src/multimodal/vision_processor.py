...
class VisionProcessor:
    ...
    async def initialize(self) -> None:
        ...
        if self._detector is None:
            ...
            self._ocr_pipeline = pipeline(
                "document-question-answering", model="impira/layoutlm-document-qa", device="cpu"
            )
            logger.info("Vision models loaded successfully")

    async def analyze_screenshot(
        self, image_data: bytes, extract_text: bool = True, detect_elements: bool = True
    ) -> ScreenshotAnalysis:
        ...
        if detect_elements and self._detector:
            ...
            analysis.interactive_elements = [
                e for e in elements if e.element_type in ["button", "input", "link", "select"]
            ]
    ...
    async def _extract_text_regions(self, image: Image.Image) -> List[VisualElement]:
        ...
        try:
            ...
            results = self._ocr_pipeline(image, question="What text is visible in this image?")
    ...
    def _generate_layout_summary(self, analysis: ScreenshotAnalysis) -> str:
        ...
        if analysis.elements:
            ...
            elements_str = ", ".join(
                [f"{count} {elem_type}" for elem_type, count in element_counts.items()]
            )
