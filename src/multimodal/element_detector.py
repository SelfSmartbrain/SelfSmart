...
class ElementDetector:
    ...
    async def detect_buttons(
        self, image: Image.Image, min_confidence: float = 0.5
    ) -> List[VisualElement]:
        ...
        for contour in contours:
            ...
            if 0.5 <= aspect_ratio <= 5.0 and w > 30 and h > 20:
                button = VisualElement(
                    ...
                    attributes={"width": w, "height": h, "aspect_ratio": aspect_ratio},
                )
    ...
    async def detect_inputs(self, image: Image.Image) -> List[VisualElement]:
    ...
    async def detect_links(self, image: Image.Image) -> List[VisualElement]:
    ...
    async def detect_all_elements(self, image: Image.Image) -> List[VisualElement]:
        ...
        filtered_elements = [
            elem for elem in merged_elements if elem.confidence >= self.config.min_confidence
        ]

        # Filter by target element types
        target_elements = [
            elem for elem in filtered_elements if elem.element_type in self.config.target_elements
        ]

        logger.info(f"Total elements detected: {len(target_elements)}")
        return target_elements

    def _merge_overlapping_elements(self, elements: List[VisualElement]) -> List[VisualElement]:
        ...
        for elem in sorted_elements:
            ...
            for merged_elem in merged:
                if (
                    self._calculate_overlap(elem.bbox, merged_elem.bbox)
    ...
    async def get_element_at_position(
        self, elements: List[VisualElement], x: int, y: int
    ...
    async def get_clickable_elements(self, elements: List[VisualElement]) -> List[VisualElement]:
