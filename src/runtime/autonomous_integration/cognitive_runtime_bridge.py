...
class CognitiveRuntimeBridge:
    ...
    async def start(self) -> None:
        ...
        if self.config.mode in (BridgeMode.PROACTIVE, BridgeMode.CONTINUOUS):
            self._tasks.append(asyncio.create_task(self.runtime.execution_loop.run()))
    ...
    async def _background_thinking_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                # Check if idle
                if (
                    self.state.idle_since
                    and (datetime.utcnow() - self.state.idle_since).total_seconds()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._stats,
            ...
            "uptime_seconds": (datetime.utcnow() - self.state.minute_start).total_seconds(),
        }
