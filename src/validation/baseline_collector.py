...
DATABASE_URL = (
    ...
    else "postgresql://postgres:password@localhost:5432/modelx"
)
...
def collect_baseline(duration_seconds: int = 300) -> Dict[str, Any]:
    ...
    se = SearchEngine(
        db_session=None, working_mem=wm, semantic_mem=None
    )  # db_session left None for baseline
...
def collect_comprehensive_baseline(
    duration_seconds: int = 300, num_goals: int = 5
) -> List[Dict[str, Any]]:
    """
    ...
    if all_results:
        for key in all_results[0].keys():
            if key != "goal":
                values = [
                    r.get(key, 0) for r in all_results if isinstance(r.get(key), (int, float))
                ]
                if values:
