...
class ToolEvolutionEngine:
    ...
    async def evaluate_tool(self, tool_name: str) -> Dict[str, Any]:
        ...
        if len(stats.recent_records) < 10:
            return {"tool_name": tool_name, "action": "none", "reason": "insufficient_data"}

        recent_success_rate = sum(1 for r in stats.recent_records if r.success) / len(
        ...
        if recent_success_rate < self.success_threshold and stats.status == "active":
            ...
            logger.warning(
                f"Tool {tool_name} success rate ({recent_success_rate:.2f}) fell below threshold. Triggering evolution."
            )
            return {
                "tool_name": tool_name,
                "action": "evolve",
