...
class ExperimentExecutor:
    ...
    async def _gather_historical_evidence(
        self, conditions: Dict[str, Any], sample_size: int
    ) -> List[ExecutionLog]:
        """
        ...
        for _ in range(sample_size):
            ...
            logs.append(
                ExecutionLog(
                    id=uuid.uuid4(), details=conditions, outcome_metric=random.gauss(base, 10.0)
                )
            )
        return logs
    ...
    async def execute_experiment(self, experiment: Experiment) -> ExperimentRun:
        ...
        control_logs = await self._gather_historical_evidence(
            experiment.control_group.conditions, experiment.control_group.sample_size_target
        )

        treatment_logs = await self._gather_historical_evidence(
            experiment.treatment_group.conditions, experiment.treatment_group.sample_size_target
        )

        control_mean = (
            sum(log.outcome_metric for log in control_logs) / len(control_logs)
            if control_logs
            else 0.0
        )
        treatment_mean = (
            sum(log.outcome_metric for log in treatment_logs) / len(treatment_logs)
            if treatment_logs
            else 0.0
        )
        ...
        conclusion = (
            "Hypothesis Validated: Significant improvement observed."
            if p_value < 0.05
            else "Hypothesis Rejected: No significant improvement."
        )
