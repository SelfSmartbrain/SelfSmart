...
class CausalReasoner:
    ...
    async def infer_causal_links(
        ...
        logger.info(
            f"Inferring causal links for {len(recent_events)} events and {len(execution_logs)} logs."
        )
        try:
            links: List[CausalLink] = []
    ...
    async def evaluate_causal_strength(
        self, link: CausalLink, new_evidence: Dict[str, Any]
    ) -> CausalLink:
    ...
    async def detect_spurious_correlations(
        ...
        try:
            ...
            for link in links:
                ...
                is_spurious = (
                    has_confounder
                    or temporal_score < 0.3
                    or statistical_significance < 0.05
                    or (link.strength < 0.3 and link.relation_type == "CORRELATES_WITH")
                )
    ...
    def _compute_statistical_significance(
        self, link: CausalLink, logs: List[Dict[str, Any]]
    ) -> float:
        ...
        source_occurrences = sum(
            1 for log in logs if any(e.get("id") == source_id for e in log.get("events", []))
        )
        joint_occurrences = sum(
            1
            for log in logs
            if source_id in [e.get("id") for e in log.get("events", [])]
            and target_id in [e.get("id") for e in log.get("events", [])]
        )
