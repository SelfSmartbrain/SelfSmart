...
async def create_benchmark_run(
    run_in: BenchmarkRunCreate, db: AsyncSession = Depends(get_db_session)
) -> BenchmarkRun:
...
async def get_benchmark_run(
    run_id: UUID, db: AsyncSession = Depends(get_db_session)
) -> BenchmarkRun:
...
async def update_benchmark_run(
    run_id: UUID, run_in: BenchmarkRunUpdate, db: AsyncSession = Depends(get_db_session)
) -> BenchmarkRun:
...
async def create_benchmark_metric(
    metric_in: BenchmarkMetricCreate, db: AsyncSession = Depends(get_db_session)
) -> BenchmarkMetric:
...
@router.post(
    "/strategies/compare", response_model=StrategyComparison, status_code=status.HTTP_201_CREATED
)
async def create_strategy_comparison(
    comparison_in: StrategyComparisonCreate, db: AsyncSession = Depends(get_db_session)
) -> StrategyComparison:
...
@router.post(
    "/skills/validate", response_model=SkillValidation, status_code=status.HTTP_201_CREATED
)
async def create_skill_validation(
    validation_in: SkillValidationCreate, db: AsyncSession = Depends(get_db_session)
) -> SkillValidation:
