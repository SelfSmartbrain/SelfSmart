...
async def create_program(
    program_in: ProgramCreate, session: Any = Depends(get_db_session)
) -> ProgramInDB:
...
async def list_programs(
    skip: int = 0, limit: int = 100, session: Any = Depends(get_db_session)
) -> List[ProgramInDB]:
...
async def get_program(program_id: UUID, session: Any = Depends(get_db_session)) -> ProgramInDB:
...
async def update_program(
    program_id: UUID, program_update: ProgramUpdate, session: Any = Depends(get_db_session)
) -> ProgramInDB:
...
async def delete_program(program_id: UUID, session: Any = Depends(get_db_session)) -> None:
