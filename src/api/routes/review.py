...
async def create_review(
    review_in: ReviewCreate, session: Any = Depends(get_db_session)
) -> ReviewInDB:
...
async def get_review(review_id: UUID, session: Any = Depends(get_db_session)) -> ReviewInDB:
...
async def update_review(
    review_id: UUID, review_update: ReviewUpdate, session: Any = Depends(get_db_session)
) -> ReviewInDB:
...
async def delete_review(review_id: UUID, session: Any = Depends(get_db_session)) -> None:
