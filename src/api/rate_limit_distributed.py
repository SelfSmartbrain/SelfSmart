...
RATE_LIMIT_REQUESTS = Counter(
    "rate_limit_requests_total", "Total rate limit checks", ["limit_type", "result"]
)

RATE_LIMIT_CURRENT = Counter(
    "rate_limit_current_usage", "Current rate limit usage", ["limit_type", "key"]
)
...
TIER_CONFIGS = {
    RateLimitTier.FREE: RateLimitConfig(
        requests_per_minute=20, requests_per_hour=500, requests_per_day=2000, burst_allowance=5
    ),
    RateLimitTier.BASIC: RateLimitConfig(
        requests_per_minute=60, requests_per_hour=1500, requests_per_day=10000, burst_allowance=20
    ),
    RateLimitTier.PRO: RateLimitConfig(
        requests_per_minute=200, requests_per_hour=5000, requests_per_day=50000, burst_allowance=50
    ),
...
ENDPOINT_OVERRIDES = {
    "/api/auth/login": RateLimitConfig(
        requests_per_minute=5, requests_per_hour=20, requests_per_day=50, burst_allowance=2
    ),
    "/api/auth/register": RateLimitConfig(
        requests_per_minute=3, requests_per_hour=10, requests_per_day=30, burst_allowance=2
    ),
    "/api/chat": RateLimitConfig(
        requests_per_minute=30, requests_per_hour=1000, requests_per_day=5000, burst_allowance=10
    ),
}
...
class DistributedRateLimiter:
    ...
    async def check_rate_limit(
        self, key: str, config: RateLimitConfig, window: str = "minute"
    ) -> tuple[bool, dict]:
        """
        ...
        RATE_LIMIT_REQUESTS.labels(
            limit_type=window, result="allowed" if allowed else "denied"
        ).inc()

        if allowed:
            RATE_LIMIT_CURRENT.labels(
                limit_type=window, key=key[:20]  # Truncate for cardinality
            ).inc()
    ...
    async def check_request(
        self, request: Request, user_id: Optional[str] = None, api_key: Optional[str] = None
    ) -> tuple[bool, dict]:
        """
...
async def check_rate_limit_dependency(
    request: Request, user_id: Optional[str] = None, api_key: Optional[str] = None
) -> None:
