...
class SocietyRuntime:
    ...
    async def create_society(
        ...
        for member_id in society.members:
            role = (
                SocietyRole.SUPERVISOR
                if supervisors and member_id in supervisors
                else SocietyRole.WORKER
            )
        ...
        logger.info(
            f"Created society {society_id}: {name} with {len(society.members)} members "
    ...
    async def delegate_task(
        ...
        if delegator_role not in (SocietyRole.SUPERVISOR, SocietyRole.COORDINATOR):
            logger.warning(
                f"Agent {assigned_by} lacks delegation authority (role: {delegator_role.value})"
            )
            return None
        ...
        if assigned_to is None:
            ...
            if self.delegation_protocol and self.agent_registry:
                from src.cognition.delegation_protocol import (
                    DelegationTask as ProtoDelegationTask,
                    DelegationPriority,
                )
        ...
        logger.info(
            f"Delegated task {task_id} from {assigned_by} to {assigned_to} in society {society_id}"
        )
    ...
    async def complete_task(
        ...
        if completed_by and task.assigned_to != completed_by:
            logger.warning(
                f"Task {task_id} completed by {completed_by} but assigned to {task.assigned_to}"
            )
            return False
    ...
    def get_agent_societies(self, agent_id: str) -> List[Society]:
        ...
        return [self._societies[sid] for sid in society_ids if sid in self._societies]
    ...
    def get_active_collaborations(self) -> List[Collaboration]:
        ...
        return [collab for collab in self._collaborations.values() if collab.status == "active"]
    ...
    def get_agent_delegations(
        self, agent_id: str, as_assigner: bool = True
    ) -> List[DelegationTask]:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_societies": sum(
                1 for s in self._societies.values() if s.status == SocietyStatus.ACTIVE
            ),
