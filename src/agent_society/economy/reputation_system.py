...
class ReputationProfile:
    ...
    peer_endorsements: Dict[str, Dict[ReputationDimension, Decimal]] = field(default_factory=dict)  # endorser -> dimension -> score
    ...
    def get_dimension_percentile(self, dimension: ReputationDimension) -> Decimal:
        ...
        return min(Decimal("100"), max(Decimal("0"), self.get_dimension_score(dimension) * Decimal("100")))
...
class ReputationSystem:
    ...
    def __init__(
        ...
        self._pending_verifications: Dict[str, List[str]] = defaultdict(list)  # event_id -> verifier_ids
    ...
    def record_event(
        ...
        logger.debug(f"Reputation event: {agent_id} {dimension.value} {delta:+.3f} (weight: {weight})")
        return event
    ...
    def endorse(
        ...
        if endorser_profile.get_dimension_score(dimension) < Decimal("0.5"):
            logger.warning(f"Endorser {endorser_id} has insufficient reputation in {dimension.value}")
            return False
        ...
        endorsee_profile.scores[dimension] = max(Decimal("0"), min(Decimal("1"), current + adjustment))
    ...
    def verify_event(self, event_id: str, verifier_id: str) -> bool:
        ...
        profile.scores[event.dimension] = max(Decimal("0"), min(Decimal("1"), 
            profile.scores.get(event.dimension, Decimal("0.5")) + bonus))
    ...
    def apply_decay(self, agent_id: Optional[str] = None):
        ...
        for aid in agents:
            ...
            profile.events = [e for e in profile.events if datetime.fromtimestamp(e.timestamp) > cutoff]
    ...
    def _recalculate_scores(self, profile: ReputationProfile):
        ...
        for event in profile.events:
            ...
            if age > self.decay_period:
                decay_factor = Decimal("1") - self.decay_rate * Decimal(age.days / self.decay_period.days)
            ...
            current = profile.scores.get(event.dimension, Decimal("0.5"))
            profile.scores[event.dimension] = max(Decimal("0"), min(Decimal("1"), current + adjusted_delta))
        ...
        for endorser_id, endorsements in profile.peer_endorsements.items():
            ...
            for dimension, score in endorsements.items():
                ...
                profile.scores[dimension] = max(Decimal("0"), min(Decimal("1"), current + adjustment))
    ...
    def get_percentile(self, agent_id: str, dimension: Optional[ReputationDimension] = None) -> Decimal:
    ...
    def get_leaderboard(self, dimension: Optional[ReputationDimension] = None, top_n: int = 10) -> List[Dict[str, Any]]:
    ...
    def export_profile(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "dimension_percentiles": {dim.value: float(profile.get_dimension_percentile(dim)) for dim in ReputationDimension},
            "total_events": profile.total_events,
