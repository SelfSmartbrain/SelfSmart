...
class Proposal:
    ...
    def has_quorum(self) -> bool:
        if self.total_voting_power == 0:
            return False
        participation = (self.yes_votes + self.no_votes + self.abstain_votes) / self.total_voting_power
...
class EconomicGovernance:
    ...
    def __init__(
        ...
        self._agent_votes: Dict[str, Dict[str, Vote]] = defaultdict(dict)  # agent_id -> proposal_id -> Vote
        
        # Delegation
        self._delegations: Dict[str, str] = {}  # delegator -> delegatee
        self._delegation_chains: Dict[str, List[str]] = defaultdict(list)  # delegatee -> [delegators]
    ...
    async def create_proposal(
        ...
        active_count = sum(1 for pid in self._agent_proposals.get(proposer_id, set())
        ...
        if proposal.status == ProposalStatus.VOTING:
            ...
            if proposal.voting_end and now >= proposal.voting_end:
                ...
                if proposal.has_quorum:
                    ...
                    required = self.config.supermajority_threshold if proposal.proposal_type in [
        ...
        try:
            ...
            if success:
                ...
                if proposal.stake_locked and self.token_system:
                    await self.token_system.transfer("governance_stake", proposal.proposer_id, proposal.stake_amount)
            ...
            if proposal.proposal_type == ProposalType.PARAMETER_CHANGE:
                ...
                if self.token_system:
                    for recipient, amount in proposal.parameters.get("allocations", {}).items():
                        await self.token_system.mint(recipient, Decimal(str(amount)), "treasury_allocation")
                return True
                
            elif proposal.proposal_type == ProposalType.TREASURY_SPEND:
                if self.token_system:
                    for recipient, amount in proposal.parameters.get("payments", {}).items():
                        await self.token_system.transfer("treasury", recipient, Decimal(str(amount)))
                return True
