...
class TheoryValidator:
    ...
    def validate_theory(
        ...
        if match_rate >= 0.9:
            ...
            theory.counterexamples.extend(
                [
                    obs
                    for obs in new_observations
                    if not self._observation_matches_prediction(obs, theory.predictions)
                ]
            )
        else:
        ...
        logger.info(
            f"Validated theory {theory.id}: {result.value} (confidence {confidence_before:.2f} -> {new_confidence:.2f})"
        )
        return report
