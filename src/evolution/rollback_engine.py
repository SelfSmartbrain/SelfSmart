...
class RollbackEngine:
    ...
    async def monitor_canary(
        ...
        logger.info(
            f"Monitoring canary deployment {deployment_id}, failure rate: {current_failure_rate:.4f}"
        )
        ...
        if current_failure_rate > threshold_failure_rate:
            ...
            logger.warning(
                f"Failure rate spike detected! {current_failure_rate:.4f} > {threshold_failure_rate:.4f} ({severity} severity). Rolling back."
            )
    ...
    async def execute_rollback(self, rollback: ArchitectureRollback) -> bool:
        ...
        """
        logger.info(
            f"Executing rollback for deployment {rollback.deployment_id} to version {rollback.reverted_to_version_id}"
        )
