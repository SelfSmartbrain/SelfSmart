...
class PromotionEngine:
    ...
    async def evaluate_for_promotion(
        ...
        failure_rate_improvement = baseline_failure_rate - candidate_failure_rate  # Positive means candidate is better
        stability_improvement = failure_rate_improvement / max(baseline_failure_rate, 0.001)
        
        # Normalize factors to 0-1 range for scoring
        fitness_score = min(fitness_improvement_factor / 0.5, 1.0)  # Cap at 50% improvement for full score
        failure_score = max(0, min(failure_rate_improvement / 0.1 + 0.5, 1.0))  # Centered around 0 improvement
        ...
        meets_fitness_threshold = improvement >= self.promotion_criteria["fitness_improvement_threshold"]
        meets_failure_criteria = candidate_failure_rate <= baseline_failure_rate
        meets_confidence_threshold = decision_score >= self.promotion_criteria["confidence_threshold"]
        
        # For backward compatibility with tests, also check the original simple criteria
        original_should_promote = (candidate_fitness > baseline_fitness * 1.05 and 
                                 candidate_failure_rate < baseline_failure_rate)
        
        should_promote = meets_fitness_threshold and meets_failure_criteria and meets_confidence_threshold
        ...
        if should_promote:
            logger.info(f"Candidate {candidate_id} promoted! Improvement: {improvement:.2%}, Decision score: {decision_score:.3f}")
            ...
            logger.info(f"Candidate {candidate_id} not promoted. Improvement: {improvement:.2%}, Decision score: {decision_score:.3f}")
            return None
