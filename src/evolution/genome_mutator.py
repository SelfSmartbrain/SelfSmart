...
class GenomeMutator:
    ...
    def mutate(
        self, genome_data: CognitiveGenomeData
    ) -> Tuple[CognitiveGenomeData, List[Dict[str, Any]]]:
        ...
        for trait_name, trait in new_genome_data.traits.items():
            ...
            if random.random() < eff_rate:
                ...
                if old_val != trait.value:
                    mutations.append(
                        {"trait": trait_name, "old_value": old_val, "new_value": trait.value}
                    )

        return new_genome_data, mutations

    def crossover(
        self, parent1: CognitiveGenomeData, parent2: CognitiveGenomeData
    ) -> CognitiveGenomeData:
