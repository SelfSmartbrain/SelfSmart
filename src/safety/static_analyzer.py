...
class StaticAnalyzer:
    ...
    def __init__(self, config: Optional[AnalysisConfig] = None):
        ...
        logger.info(
            f"StaticAnalyzer initialized (bandit: {self._bandit_available}, "
    ...
    def analyze_file(self, file_path: str) -> AnalysisResult:
        ...
        execution_time = (
            datetime.now() - datetime.fromtimestamp(start_time.timestamp())
        ).total_seconds()
            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        ...
                        if any(
                            secret in name
                            for secret in ["password", "secret", "key", "token", "api"]
                        ):
    ...
    def _analyze_self_modification(
    ...
    def _calculate_nesting_depth(self, node: ast.AST, tree: ast.AST) -> int:
        ...
        while hasattr(current, "parent") and current.parent:
            if isinstance(
                current.parent, (ast.If, ast.For, ast.While, ast.Try, ast.With, ast.AsyncWith)
            ):
                depth += 1
