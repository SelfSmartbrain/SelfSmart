...
class ExperienceRecorder:
                ...
                for record in records:
                    ...
                    """,
                        (
                            ...
                            (
                                json.dumps(record.execution_trace.to_dict())
                                if record.execution_trace
                                else None
                            ),
                            (
                                json.dumps(record.test_results.to_dict())
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self._db_path) as conn:
            ...
            # Success rate
            success_rate = (
                conn.execute("SELECT AVG(CAST(success AS REAL)) FROM experiences").fetchone()[0]
                or 0
            )
            ...
            # Average score
            avg_score = conn.execute("SELECT AVG(score) FROM experiences").fetchone()[0] or 0
