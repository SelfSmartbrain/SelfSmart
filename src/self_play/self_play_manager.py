...
from .sandbox_integration import SandboxTestRunner, SandboxConfig, TestCase, TestResult, SandboxExecutionResult
        ...
        try:
            ...
            if not problem:
                ...
                return IterationResult(
                    ...
                    solution_code="",
                    execution_result=SandboxExecutionResult(status=SandboxStatus.FAILED, error="No problem"),
            ...
            success = execution_result.status == SandboxStatus.COMPLETED and test_suite_result.all_passed
...
Return ONLY the solution code, no explanation."""
    ...
    async def _run_tests(
        ...
        for r in execution_result.test_results:
            test_results.append(RecorderTestResult(
                test_id=r.test_name,
                name=r.test_name,
                passed=r.passed,
    ...
    async def _extract_learning_signals(
        ...
        signals = {
            ...
            "test_pass_rate": test_suite_result.passed_tests / max(test_suite_result.total_tests, 1),
    ...
    def _analyze_code(self, code: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "complexity_estimate": len([l for l in non_empty if any(kw in l for kw in ["if", "for", "while", "try", "except"])]),
        }
    ...
    async def _record_experience(
        ...
        trace = ExecutionTrace(
            ...
            error_type=self._classify_error(execution_result.error or "") if execution_result.error else None,
