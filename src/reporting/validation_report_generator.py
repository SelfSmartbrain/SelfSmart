...
class ValidationReportGenerator:
    ...
    async def generate_daily_report(
        self, date: datetime, benchmark_runs: List[Any], validation_results: List[Any]
    ) -> ValidationReport:
        start_date = date.replace(hour=0, minute=0, second=0, microsecond=0)
        end_date = start_date + timedelta(days=1)
        return await self.generate_report(
            "Daily", start_date, end_date, benchmark_runs, validation_results
        )
    ...
    async def generate_weekly_report(
        ...
        return await self.generate_report(
            "Weekly", start_date, end_date, benchmark_runs, validation_results
        )
    ...
    async def generate_monthly_report(
        ...
        if date.month == 12:
            end_date = date.replace(
                year=date.year + 1, month=1, day=1, hour=0, minute=0, second=0, microsecond=0
            )
        else:
            end_date = date.replace(
        ...
        return await self.generate_report(
            "Monthly", start_date, end_date, benchmark_runs, validation_results
        )
