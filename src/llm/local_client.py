...
class BaseLocalClient(ABC):
    ...
    async def chat_stream(
        self, messages: List[Message], temperature: float = 0.7, max_tokens: int = 2000, **kwargs
    ...
    async def initialize(self) -> None:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            try:
                ...
                raise RuntimeError(
                    f"Cannot connect to Ollama at {self.base_url}. " f"Start with: ollama serve"
                )
...
class LlamaCppClient(BaseLocalClient):
    ...
    def __init__(self, model: str, base_url: str = "http://localhost:8080", **kwargs):
        super().__init__(model, **kwargs)
        self.base_url = base_url.rstrip("/")
        self.session: Optional[aiohttp.ClientSession] = None
...
class TransformersClient(BaseLocalClient):
    ...
    def __init__(self, model: str, device: str = "auto", torch_dtype: str = "auto", **kwargs):
        super().__init__(model, **kwargs)
        self.device = device
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                "transformers not installed. pip install transformers torch accelerate"
            )
        except Exception as e:
    ...
    async def chat(
        ...
        **kwargs,
        ...
        prompt = self._tokenizer.apply_chat_template(
            self._to_api_messages(messages), tokenize=False, add_generation_prompt=True
        )
...
class LocalModelClient:
    ...
    @classmethod
    async def create(
        cls, model: str, backend: Union[LocalBackend, str] = LocalBackend.OLLAMA, **kwargs
    ) -> "LocalModelClient":
    ...
    async def initialize(
        self, model: str, backend: Union[LocalBackend, str] = LocalBackend.OLLAMA, **kwargs
    ) -> None:
