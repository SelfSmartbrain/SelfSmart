...
class RollbackManager:
    ...
    async def _load_history(self) -> None:
        ...
        if history_file.exists():
            try:
                ...
                logger.info(
                    f"Loaded {len(self._deployment_history)} deployments, "
    ...
    async def rollback_deployment(
        ...
        try:
            ...
            if backup_path.exists():
                ...
                success = True
                logger.info(
                    f"Rolled back deployment {deployment_id} from backup {deployment.backup_id}"
                )
            else:
    ...
    async def rollback_specific_patches(
        ...
        matching_patches = [
            p for p in self._last_deployment.patches if p.get("patch_id") in patch_ids
        ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "successful_deployments": len(
                [d for d in self._deployment_history if d.status == "success"]
            ),
            "rolled_back_deployments": len(
                [d for d in self._deployment_history if d.status == "rolled_back"]
            ),
