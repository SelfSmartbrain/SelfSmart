...
class RunValidationTool(AgentTool):
    ...
    description: str = (
        "Run the full Phase V2 validation suite including all ablation studies, coding benchmark, cost analysis, and long-horizon testing. Returns summary of key findings."
    )
    ...
    async def _execute(self, quick: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        summary = await asyncio.get_event_loop().run_in_executor(None, runner.run_all_validations)

        findings = summary.get("key_findings", [])

        return {"summary": summary, "findings": findings, "quick": quick}


class RunAblationArgs(BaseModel):
    component: str = Field(
        description="Component to ablate: memory, concepts, world_model, governance"
    )
...
class RunAblationTool(AgentTool):
    ...
    description: str = (
        "Run ablation study for a specific ModelX component. Component options: memory, concepts, world_model, governance."
    )
    ...
    async def _execute(self, component: str, **kwargs) -> Dict[str, Any]:
        ...
        result = await asyncio.get_event_loop().run_in_executor(None, method_map[component])
...
class RunBenchmarkTool(AgentTool):
    ...
    description: str = (
        "Run coding capability benchmark against task types: bug_fixing, feature_implementation, test_generation, refactoring, repository_analysis."
    )
    ...
    async def _execute(self, task_type: str = "all", **kwargs) -> Dict[str, Any]:
        ...
        runner = ValidationRunner(output_dir=Path("validation_results"))
        result = await asyncio.get_event_loop().run_in_executor(None, runner.run_coding_benchmark)
...
class RunCostAnalysisTool(AgentTool):
    ...
    description: str = (
        "Analyze computational costs (tokens, latency, CPU, memory, API cost) across ModelX subsystems: memory, reasoning, world_model, governance."
    )
    ...
    async def _execute(self, **kwargs) -> Dict[str, Any]:
        ...
        runner = ValidationRunner(output_dir=Path("validation_results"))
        result = await asyncio.get_event_loop().run_in_executor(None, runner.run_cost_analysis)
...
class RunLongHorizonTool(AgentTool):
    ...
    description: str = (
        "Run extended autonomy test (24h, 72h, 1 week simulation) tracking goal persistence, failure recovery, memory growth, knowledge growth, and stability."
    )
    ...
    async def _execute(self, duration_hours: int = 1, **kwargs) -> Dict[str, Any]:
        ...
        runner = ValidationRunner(output_dir=Path("validation_results"))
        result = await asyncio.get_event_loop().run_in_executor(None, runner.run_long_horizon_test)
...
class GetValidationReportTool(AgentTool):
    ...
    description: str = (
        "Retrieve the latest Phase V2 validation report with ablation results, benchmark scores, cost analysis, and long-horizon stability metrics."
    )
    ...
    async def _execute(self, **kwargs) -> Dict[str, Any]:
        ...
        return {"report": report[:3000], "summary": summary, "full_report_path": str(report_path)}
