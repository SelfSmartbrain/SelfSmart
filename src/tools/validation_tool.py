...
class RunValidationTool(AgentTool):
    ...
    description: str = (
        "Run full validation suite including memory/concept/world_model/governance ablations, coding benchmark, cost analysis, and long-horizon test"
    )
    max_retries: int = 0
    ...
    async def _execute(self, quick: bool = False) -> Dict[str, Any]:
        ...
        if quick:
            return {
                "message": "Quick validation not directly supported, running full suite",
...
class RunAblationTool(AgentTool):
    ...
    description: str = (
        "Run ablation study for a specific component (memory, concepts, world_model, governance)"
    )
    max_retries: int = 0
    ...
    async def _execute(self, component: str) -> Dict[str, Any]:
        ...
        if component == "memory":
            ...
            return {
                "error": f"Unknown component: {component}. Use: memory, concepts, world_model, governance"
            }
...
class RunCostAnalysisTool(AgentTool):
    ...
    description: str = (
        "Analyze computational costs (tokens, latency, CPU, memory, API cost) across ModelX subsystems"
    )
    max_retries: int = 0
...
class RunLongHorizonTool(AgentTool):
    ...
    description: str = (
        "Run extended autonomy test (24h, 72h, 1 week) tracking goal persistence, failure recovery, memory/knowledge growth, and stability"
    )
    max_retries: int = 0
