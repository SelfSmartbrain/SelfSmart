...
class DecisionTracker:
    ...
    def get_decision_ratio(self, window_minutes: Optional[int] = None) -> Dict[str, float]:
        ...
        augmented_count = sum(
            1 for d in recent_decisions if d.decision_type == DecisionType.AUGMENTED
        )
        delegated_count = sum(
            1 for d in recent_decisions if d.decision_type == DecisionType.DELEGATED
        )
    ...
    def get_decision_ratio_by_domain(self, domain: DecisionDomain) -> Dict[str, float]:
        ...
        augmented_count = sum(
            1 for d in domain_decisions if d.decision_type == DecisionType.AUGMENTED
        )
        delegated_count = sum(
            1 for d in domain_decisions if d.decision_type == DecisionType.DELEGATED
        )
    ...
    def get_decision_statistics(self) -> Dict[str, Any]:
        ...
        # Count by type
        by_type = {dt.value: len(self.decisions_by_type.get(dt, [])) for dt in DecisionType}

        # Count by domain
        by_domain = {dd.value: len(self.decisions_by_domain.get(dd, [])) for dd in DecisionDomain}
