...
class ChallengeGenerator(BaseModel):
    ...
    async def generate_challenge(self, hypothesis_id: UUID, flaws: List[str]) -> Challenge:
        ...
        desc = (
            f"Address the following flaws: {', '.join(flaws)}"
            if flaws
            else "Stress test the hypothesis under edge cases."
        )
        difficulty = 0.5 + min(0.5, len(flaws) * 0.1)

        return Challenge(hypothesis_id=hypothesis_id, description=desc, difficulty_level=difficulty)
