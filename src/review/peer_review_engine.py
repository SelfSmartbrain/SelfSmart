...
class PeerReviewEngine(BaseModel):
    ...
    async def run_review_cycle(
        self, hypothesis_id: UUID, content: str, evidence: List[str], methodology: str
    ) -> ReviewCycleResult:
        ...
        challenge = await self.challenger.generate_challenge(
            hypothesis_id, critique.flaws_identified
        )

        # 3. Verification attempt
        verification = await self.verifier.verify(
            hypothesis_id, methodology, critique.replicability_score
        )

        status = (
            "Accepted"
            if verification.is_verified and critique.evidence_strength > 0.6
            else "Needs Revision"
        )
