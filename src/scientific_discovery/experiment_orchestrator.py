...
class ExperimentOrchestrator:
    ...
    def design_experiment(
        ...
        logger.info(
            f"Designed experiment {experiment.experiment_id} for hypothesis: {hypothesis[:50]}..."
        )
        return experiment
    ...
    def get_queued_experiments(self) -> List[Experiment]:
        ...
        return [self.experiments[eid] for eid in self.experiment_queue if eid in self.experiments]
    ...
    def get_completed_experiments(self) -> List[Experiment]:
        ...
        return [
            exp for exp in self.experiments.values() if exp.status == ExperimentStatus.COMPLETED
        ]

    def get_experiments_by_hypothesis(self, hypothesis: str) -> List[Experiment]:
        """Get experiments for a specific hypothesis."""
        return [exp for exp in self.experiments.values() if exp.hypothesis == hypothesis]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for experiment in self.experiments.values():
            status_counts[experiment.status.value] = (
                status_counts.get(experiment.status.value, 0) + 1
            )
