...
class DiscoveryLoop:
    ...
    async def observation_phase(self, observations: List[str]) -> None:
        ...
        self.knowledge_base["observations"] = (
            self.knowledge_base.get("observations", []) + observations
        )
    ...
    async def hypothesis_generation_phase(self) -> List[str]:
        ...
        if self.world_model and hasattr(self.world_model, "generate_hypotheses"):
            ...
            hypotheses = self._generate_simple_hypotheses(self.current_iteration.observations)
    ...
    async def theory_formation_phase(self) -> List[str]:
        ...
        # Form theories from successful experiments
        successful_results = [
            r for r in self.current_iteration.results if r["outcome"] == "success"
        ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hypotheses_per_iteration": (
                total_hypotheses / len(self.iterations) if self.iterations else 0.0
            ),
            "theories_per_iteration": (
                total_theories / len(self.iterations) if self.iterations else 0.0
            ),
        }
