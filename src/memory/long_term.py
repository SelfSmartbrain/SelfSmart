...
class LongTermMemory:
    ...
    async def store(
        ...
        try:
            ...
            logger.info(
                "Stored long-term memory", memory_id=str(memory_record.id), type=memory_type
            )
            ...
            logger.error(
                "Failed to store long-term memory", error=str(e), error_type=type(e).__name__
            )
    ...
    async def recall(
        ...
        try:
            ...
            return sorted(
                memories,
                key=lambda x: x["relevance_score"] + (x["importance_score"] * 0.2),
                reverse=True,
            )[:limit]
