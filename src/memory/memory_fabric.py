    ...
    async def _query_working(
        ...
        try:
            ...
            if hasattr(backend, "keys"):
                ...
                logger.warning(
                    "Redis backend lacks 'scan'; falling back to KEYS (O(N) — upgrade redis client)"
                )
                pattern = f"*{query}*"
                    ...
                    if lowered in str(key).lower() or lowered in content.lower():
                        entries.append(
                            MemoryEntry(
                                ...
                                memory_type=MemoryType.WORKING,
                                metadata=(
                                    value.get("metadata", {}) if isinstance(value, dict) else {}
                                ),
                                importance=(
                                    value.get("importance", 0.5) if isinstance(value, dict) else 0.5
                                ),
            ...
            if memory_type == MemoryType.SEMANTIC:
                ...
                if hasattr(backend, "store_memory"):
                    memory_id = await backend.store_memory(
                        user_id=uuid.UUID(
                            entry.metadata.get("user_id", "00000000-0000-0000-0000-000000000000")
                        ),
                        content=entry.content,
        ...
        try:
            if memory_type == MemoryType.SEMANTIC:
                if hasattr(backend, "delete"):
                    await backend.delete(collection_name="memories", points_selector=[memory_id])
                    self._cache.clear()
                    return True
