...
class BeliefPruner:
    ...
    async def prune_beliefs(
        ...
        actionable = [fs for fs in fitness_scores if self.determine_action(fs) != PruneAction.KEEP]
        ...
        to_quarantine = [
            fs for fs in actionable if self.determine_action(fs) == PruneAction.QUARANTINE
        ]
            ...
            if self.neo4j_client:
                try:
                    ...
                    await self.neo4j_client.execute_query(
                        cypher, {"id": belief_id, "time": datetime.utcnow().isoformat()}
                    )
                except Exception as e:
