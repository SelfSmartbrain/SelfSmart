...
class KnowledgeFitnessScheduler:
    ...
    async def run_pruning(self) -> List[Any]:
        ...
        try:
            ...
            logger.info(
                f"Pruning complete in {elapsed:.1f}s: " f"{success_count}/{len(results)} successful"
            )
    ...
    async def run_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            elapsed = (datetime.utcnow() - start_time).total_seconds()
            logger.info(f"Validation complete in {elapsed:.1f}s: " f"{validation_results}")
...
class ManualFitnessController:
    ...
    async def get_belief_details(self, belief_id: str) -> Optional[Dict[str, Any]]:
        ...
        if self.fitness_engine.neo4j_client:
            ...
            result = await self.fitness_engine.neo4j_client.execute_query(cypher, {"id": belief_id})
            if result:
