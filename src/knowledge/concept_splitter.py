...
class ConceptSplitter:
    ...
    def _split_domain(self, concept, num_splits: int) -> List[Dict[str, Any]]:
        ...
        for i, keyword in enumerate(domain_keywords[:num_splits]):
            ...
            relevant_memories = [
                m for m in concept.source_memories if keyword.lower() in str(m).lower()
            ]
            ...
            splits.append(
                {
                    ...
                    "source_memories": concept.source_memories[
                        : len(concept.source_memories) // num_splits
                    ],
                }
            )
    ...
    def _group_memories_semantic(self, memories: List[str], num_groups: int) -> List[List[str]]:
        ...
        for i, memory in enumerate(memories):
            ...
            for other in memories[i + 1 :]:
                ...
                overlap = len(memory_words & other_words) / max(
                    len(memory_words), len(other_words), 1
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for result in self.split_history:
            strategy_counts[result.split_strategy.value] = (
                strategy_counts.get(result.split_strategy.value, 0) + 1
            )

        return {
            "total_splits": len(self.split_history),
            "by_strategy": strategy_counts,
            "average_new_concepts": (
                sum(len(r.new_concept_ids) for r in self.split_history) / len(self.split_history)
