...
class OutcomePredictor:
    ...
    def _identify_patterns(
        ...
        if successful_outcomes:
            avg_confidence = sum(o.get("confidence", 0.5) for o in successful_outcomes) / len(
                successful_outcomes
            )
            patterns.append(f"Successful decisions have average confidence {avg_confidence:.2f}")

        if failed_outcomes:
            avg_confidence = sum(o.get("confidence", 0.5) for o in failed_outcomes) / len(
                failed_outcomes
            )
    ...
    def compare_predictions(
        ...
        comparison = {
            ...
            "high_confidence": sum(
                1 for p in predictions if p.confidence == PredictionConfidence.HIGH
            ),
            "medium_confidence": sum(
                1 for p in predictions if p.confidence == PredictionConfidence.MEDIUM
            ),
            "low_confidence": sum(
                1 for p in predictions if p.confidence == PredictionConfidence.LOW
            ),
