...
# Metrics
RETRY_ATTEMPTS = Counter("retry_attempts_total", "Total retry attempts", ["function", "outcome"])
...
async def retry_with_backoff(
    ...
    for attempt in range(config.max_attempts):
        try:
            ...
            RETRY_ATTEMPTS.labels(function=func_name, outcome="success").inc(attempt + 1)
            ...
            if config.retryable_exceptions and not isinstance(e, config.retryable_exceptions):
                RETRY_ATTEMPTS.labels(function=func_name, outcome="non_retryable").inc()
                raise
    ...
    RETRY_ATTEMPTS.labels(function=func_name, outcome="exhausted").inc(config.max_attempts)
...
def with_retry(
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        async def wrapper(*args, **kwargs) -> T:
            return await retry_with_backoff(lambda: func(*args, **kwargs), config, func.__name__)
