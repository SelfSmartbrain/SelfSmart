    ...
    async def validate_outcome(self, target_id: uuid.UUID, outcome_data: dict) -> ValidationResult:
        ...
        for criterion in criteria:
            ...
            if isinstance(value, (int, float)):
                ...
                elif criterion.endswith("_rate") or criterion.endswith("_score") or criterion.endswith("_percentage"):
                    # Assume these should be between 0 and 100 or 0 and 1
                    if value < 0 or (value > 1 and value > 100):
                        is_valid = False
                        failed_criteria.append(f"Value out of expected range for {criterion}: {value}")
