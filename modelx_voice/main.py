...
class HealthCheckRunner:
    ...
    def print_report(self):
        ...
        print("\n" + "=" * 50)
        print(
            f"Summary: {pass_count}/{total} passed, {warn_count} warnings, {fail_count} failures, {skip_count} skipped"
        )
...
def check_audio_devices() -> HealthCheck:
    ...
    try:
        ...
        if has_input and has_output:
            check.pass_(
                f"Input: {has_input}, Output: {has_output}", input=has_input, output=has_output
            )
        elif has_input or has_output:
            check.warn(
                f"Partial: Input={has_input}, Output={has_output}",
                input=has_input,
                output=has_output,
            )
        else:
...
async def check_llm_connectivity() -> HealthCheck:
    ...
    try:
        ...
        await provider.close()
        check.pass_(
            f"{config.api.provider} API responding",
            provider=config.api.provider,
            model=config.api.model,
        )
    except Exception as e:
        check.fail(f"{config.api.provider} API failed: {e}", provider=config.api.provider)
    return check
...
Examples:
  ...
  modelx --version            # Show version
    ...
    voice_parser.add_argument(
        "--download-voices", action="store_true", help="List available voices"
    )
    voice_parser.add_argument(
        "--voice", choices=["professional", "casual", "clear"], help="Voice profile"
    )
    voice_parser.add_argument(
        "--provider", choices=["anthropic", "openai", "openrouter", "ollama"], help="LLM provider"
    )
    ...
    parser.add_argument(
        "--setup", action="store_true", help="Run setup wizard (alias for 'voice --setup')"
    )
