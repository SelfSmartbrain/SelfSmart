...
class AudioPipeline:
    ...
    async def _handle_command(self, cmd: VoiceCommand) -> bool:
        ...
        if cmd.type == CommandType.STOP:
            ...
            filepath = (
                Path.home() / f"modelx_conversation_{int(asyncio.get_event_loop().time())}.json"
            )
            ...
            self.ui.print_info(
                f"Provider: {stats['provider']}, Model: {stats['model']}, Turns: {stats['turns']}, Tokens: {stats['total_tokens']}"
            )
            return True
